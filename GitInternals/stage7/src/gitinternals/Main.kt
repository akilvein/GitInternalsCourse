...
fun showTreeForCommit(gitDir: String, commit: String) {
    fun showTree(tree: String, parent: String) {
        val treePath = "$gitDir/objects/${tree.substring(0, 2)}/${tree.substring(2, 40)}"
        val tree = GitObject.parseFromFile(treePath) as GitObject.Tree

        //println("!!!!TRREE")
        //tree.print()

        for (i in tree.items) {
            val permission = i.permission.toInt(16)
            val flagFile = 0x0100000
            val flagFolder = 0x0040000

            when {
                permission and flagFile != 0 -> {
                    println("$parent${i.name}")
                }

                permission and flagFolder != 0 -> {
                    showTree(i.hash, "${i.name}/")
                }
    ...
    val commitPath = "$gitDir/objects/${commit.substring(0, 2)}/${commit.substring(2, 40)}"
    val commit = GitObject.parseFromFile(commitPath) as GitObject.Commit
